...
from django_js_reverse.core import generate_js, generate_ts_defs
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        file = 'reverse.js'
        definition_file = 'reverse.ts'
        fs = FileSystemStorage(location=location)
        if fs.exists(file):
            fs.delete(file)
        if fs.exists(definition_file):
            fs.delete(definition_file)
        ...
        definitions = bytes(generate_ts_defs(default_urlresolver), encoding="UTF8")
        fs.save(file, ContentFile(content))
        fs.save(definition_file, ContentFile(definitions))
