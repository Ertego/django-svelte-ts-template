...
def generate_ts_defs(default_urlresolver, script_prefix=None):
    ...
    for x in urls:
        ...
        ts_lines.append(f'"{x[0]}": ({", ".join(args)}): string => {{return ""}},')

    # language=TypeScript
    return """export let Urls = {
        """ + "\n    ".join(ts_lines) + """
    }"""
...
def generate_js(default_urlresolver):
    ...
    if not isinstance(minfiy, bool):
        raise ImproperlyConfigured(
            'JS_REVERSE_JS_MINIFY setting "%s" is not a valid setting. Needs to be set to True or False.' % (minfiy))
